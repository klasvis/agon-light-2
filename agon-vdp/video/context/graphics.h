...
void Context::plotLine(bool omitFirstPoint, bool omitLastPoint, bool usePattern, bool resetPattern) {
	...
	tek_draw_line(p2.X, p2.Y, p1.X, p1.Y, canvas->getWidth(), canvas->getHeight(), gfg.R, gfg.G, gfg.B);
}
...
void Context::plotPoint() {
	...
	tek_draw_point(p1.X, p1.Y, canvas->getWidth(), canvas->getHeight(), gfg.R, gfg.G, gfg.B);
}
...
void Context::plotCircle(bool filled) {
	...
	if (r > 0) {
		int sw = canvas->getWidth(), sh = canvas->getHeight();
		tek_draw_circle(p2.X, p2.Y, r, sw, sh, gfg.R, gfg.G, gfg.B, filled);
	}
}
