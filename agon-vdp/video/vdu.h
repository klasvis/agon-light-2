...
void VDUStreamProcessor::vdu(uint8_t c, bool usePeek) {
	if (tekDecoder.isActive()) {
		if (c == 0x17) { // VDU 23 - could be VDU 23, 0, &FC, 0
			if (peekByte_t(5) == 0x00) {
				readByte_t(); // consume 0x00
				if (peekByte_t(5) == VDP_TEKTRONIX) {
					readByte_t(); // consume VDP_TEKTRONIX
					auto b3 = readByte_t();
					if (b3 == 0) {
						tekDecoder.stop();
						if (consoleMode) {
							DBGSerial.write(0x1F); // US: Return to Alpha mode
						}
						return;
					}
				}
