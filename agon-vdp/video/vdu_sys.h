...
void VDUStreamProcessor::vdu_sys_video() {
	...
	switch (mode) {
		...
		case VDP_TEKTRONIX: {			// VDU 23, 0, &FC, mode [, r, g, b]
			auto m = readByte_t();
			if (m == 0) {
				tekDecoder.stop();
				if (consoleMode) DBGSerial.write(0x1F); // US: Alpha mode
			} else if (m == 1) {
				tekDecoder.start(false);
			} else if (m == 2) {
				tekDecoder.start(true);
				if (consoleMode) {
					DBGSerial.write(0x1B); // ESC
					DBGSerial.write(0x0C); // FF: Page Erase
				}
			} else if (m == 3) {
				auto r = readByte_t();
				auto g = readByte_t();
