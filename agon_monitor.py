...
def main():
    ...
    if args.file:
        ...
        import serial
        print(f"[VDP Monitor] Opening serial port {args.port} at {args.baud} baud...")
        try:
    ...
    title = f"Agon Light 2 - Live VDP Screen Monitor [{'Wi-Fi' if is_wifi else args.port}]"
    ...
    def erase_screen(bg_col=None):
        col = bg_col if bg_col else tuple(cur_text_bg)
        canvas.fill(col)
        cursor["col"] = 0
        cursor["row"] = 0
        tek["x0"] = 0
    ...
    def handle_ext_cmd(cmd_bytes):
        ...
        try:
            if op == "C" and len(parts) >= 4:
                ...
                if len(parts) >= 4:
                    cur_text_bg[0], cur_text_bg[1], cur_text_bg[2] = int(parts[1]), int(parts[2]), int(parts[3])
                erase_screen(tuple(cur_text_bg))
            elif op == "CLG":
                if len(parts) >= 4:
                    cur_gfx_bg[0], cur_gfx_bg[1], cur_gfx_bg[2] = int(parts[1]), int(parts[2]), int(parts[3])
                ...
                pygame.draw.aaline(canvas, tuple(cur_gfx_fg), (x1, y1), (x2, y2))
            elif op == "P" and len(parts) >= 3:
                x, y = int(parts[1]), int(parts[2])
                if 0 <= x < TEK_W and 0 <= y < TEK_H:
                    canvas.set_at((x, y), tuple(cur_gfx_fg))
                ...
                pygame.draw.polygon(canvas, tuple(cur_gfx_fg), [(x1, y1), (x2, y2), (x3, y3)], 0 if filled else 1)
            elif op == "Q" and len(parts) >= 10:
                x1, y1, x2, y2, x3, y3, x4, y4 = [int(p) for p in parts[1:9]]
                filled = (int(parts[9]) != 0)
                pygame.draw.polygon(canvas, tuple(cur_gfx_fg), [(x1, y1), (x2, y2), (x3, y3), (x4, y4)], 0 if filled else 1)
                ...
                if rx > 0 and ry > 0:
                    ...
                    pygame.draw.ellipse(canvas, tuple(cur_gfx_fg), rect, 0 if filled else 1)
                ...
                for b in chunk:
                    ...
                    if b == 7:     # BEL
                        ...
                        if cursor["row"] >= c_rows:
                            ...
                            pygame.draw.rect(canvas, tuple(cur_text_bg), (0, TEK_H - int(ch), TEK_W, int(ch)))
                            cursor["row"] = c_rows - 1
                        continue
                    elif b == 27:  # ESC
                        tek["savemode"] = tek["mode"]
                        ...
                        if cursor["col"] > 0:
                            cursor["col"] -= 1
                            pygame.draw.rect(canvas, tuple(cur_text_bg), (int(cursor["col"] * cw), int(cursor["row"] * ch), int(cw) + 1, int(ch) + 1))
                        continue
                    ...
                    # Alpha Text Mode
                        ...
                        if 32 <= b <= 126:
                            ...
                            x = int(cursor["col"] * cw)
                            y = int(cursor["row"] * ch)
                            pygame.draw.rect(canvas, tuple(cur_text_bg), (x, y, int(cw) + 1, int(ch) + 1))
                            glyph = fnt.render(char, True, tuple(cur_text_fg))
